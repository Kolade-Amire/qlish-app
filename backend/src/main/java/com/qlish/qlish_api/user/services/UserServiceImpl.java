...
import com.qlish.qlish_api.exceptions.UserPointsUpdateException;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional
    public void updateUserAllTimePoints(ObjectId id, Integer testPoints) {
        try {
            User user = findUserById(id);
            Long existingPoints = user.getAllTimePoints();
            user.setAllTimePoints(existingPoints + testPoints.longValue());
        } catch (Exception e) {
    ...
    public List<UserDto> getUsersWithTop20Points() {
        ...
        return UserDto.fromUserList(topUsers);
    }
